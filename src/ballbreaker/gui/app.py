    ...
    def run(self):
        try:
            ...
            try:
                ...
                self.status_signal.emit(f"Warning: Symlink failed ({e}). Desktop entry will link directly.")
            ...
            self.finished_signal.emit(True, f"Successfully installed {self.app_name}!\n\nLauncher: {desktop_path.name}")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        subtitle_label = QLabel("Linux Tarball Installer & Desktop Shortcut Generator", self)
        ...
        self.elevate_check = QCheckBox("Request system administrative privileges (pkexec/sudo) if needed", self)
        self.elevate_check.setChecked(self.config["elevate"])
        group_layout.addWidget(self.elevate_check, 6, 0, 1, 3)
        
        # Terminal Checkbox
        self.terminal_check = QCheckBox("Run application inside a terminal window (Terminal=true)", self)
    ...
    def on_file_selected(self, file_path: Path):
        ...
        icon_candidates = [e for e in executables if e.lower().endswith(('.png', '.svg'))]
        if icon_candidates:
    ...
    def browse_target_dir(self):
        dir_path = QFileDialog.getExistingDirectory(self, "Select Install Directory", self.target_edit.text())
        if dir_path:
            self.target_edit.setText(dir_path)

    def browse_bin_dir(self):
        dir_path = QFileDialog.getExistingDirectory(self, "Select Symlink Directory", self.bin_edit.text())
        if dir_path:
            self.bin_edit.setText(dir_path)

    def browse_desktop_dir(self):
        dir_path = QFileDialog.getExistingDirectory(self, "Select Launcher Directory", self.desktop_edit.text())
        if dir_path:
    ...
    def start_install(self):
        ...
        if not exec_text or exec_text.startswith("("):
            QMessageBox.warning(self, "Validation Error", "Please select or specify a valid executable path.")
            return
