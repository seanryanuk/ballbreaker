...
def parse_args(args=None):
    ...
    parser.add_argument(
        "-t", "--tarball", required=True, type=Path, help="Path to the tarball archive."
    )
    parser.add_argument(
        "-n",
        "--name",
    ...
    parser.add_argument(
        "--icon", type=Path, help="Path to an icon file for the desktop entry."
    )
    parser.add_argument(
        "--run-in-terminal",
        action="store_true",
...
def run_cli(args=None) -> int:
    ...
    if not is_valid:
        print(
            f"Error: Invalid tarball or corrupted archive: {tarball_path}",
            file=sys.stderr,
        )
        return 1
    ...
    if not executables:
        print(
            "Warning: No executable files found in the extracted directory.",
            file=sys.stderr,
        )
        main_exec = None
    else:
        ...
        if parsed.exec_rel:
            ...
            if not full_exec_path.exists() or not os.access(full_exec_path, os.X_OK):
                print(
                    f"Error: Specified executable '{parsed.exec_rel}' not found or not executable.",
                    file=sys.stderr,
                )
                return 1
            ...
            if len(executables) > 1:
                print(
                    f"Other candidates found: {[e.as_posix() for e in executables[1:]]}"
                )
    ...
    if main_exec:
        ...
        try:
            ...
            print(
                f"Warning: Failed to create symbolic link: {e}. Desktop entry will point directly to binary.",
                file=sys.stderr,
            )
